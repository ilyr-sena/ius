...
<img id="img" draggable="false" src="http://127.0.0.1:9100/stream?fps=25&scale=0.6&q=0.45">
...
// ---- fast legacy WDA endpoints (proven ~30ms over USB) ----
async function wdaTap(x, y){
  if (!await ensureSession()) return false;
  const [code] = await api('POST', '/session/' + sid + '/wda/tap/' + x + '/' + y);
  return code >= 200 && code < 300;
}
async function wdaHold(x, y, ms){
  if (!await ensureSession()) return false;
  const [code] = await api('POST', '/session/' + sid + '/wda/touchAndHold',
                           {x, y, duration: ms / 1000});
  return code >= 200 && code < 300;
}
async function wdaDrag(x1, y1, x2, y2, durS){
  if (!await ensureSession()) return false;
  const [code] = await api('POST', '/session/' + sid + '/wda/dragfromtoforduration',
    {fromX: x1, fromY: y1, toX: x2, toY: y2, duration: durS});
  return code >= 200 && code < 300;
}

// ---- mouse -> gestures ----
let dragging=false, swiping=false, moved=false,
    sx=0, sy=0, lx=0, ly=0, t0=0, lastDragSent=0;
...
img.addEventListener('mousedown', ev => {
  ...
  [sx, sy] = mapXY(ev);
  lx = sx; ly = sy;
  dragging = true; swiping = false; moved = false; t0 = performance.now();
  lastDragSent = 0;
});
window.addEventListener('mousemove', ev => {
  if (!dragging) return;
  const [x, y] = mapXY(ev);
  if (!swiping && Math.hypot(x - sx, y - sy) > 8) {
  ...
  if (swiping) {
    ...
    if (now - lastDragSent >= 120) {
      lastDragSent = now;
      wdaDrag(lx, ly, x, y, 0.12).then(ok => {
        if (!ok) setStatus('drag segment rejected');
      });
      lx = x; ly = y;
    }
  }
...
window.addEventListener('mouseup', async ev => {
  ...
  const held = Math.min(2000, Math.max(60, Math.round(performance.now() - t0)));

  if (swiping) {                                    // finish the live drag
    const ok = await wdaDrag(lx, ly, ex, ey, 0.15);
    setStatus(ok ? 'swipe done' : 'final drag segment failed');
    return;
  }
  if (held > 550) {                                 // long-press
    const ok = await wdaHold(ex, ey, held);
    setStatus(ok ? 'long-press ok (' + held + 'ms)' : 'hold failed');
    return;
  }
  const ok = await wdaTap(ex, ey);                  // plain tap, fast path
  if (!ok) {                                        // W3C fallback
    await sendActions([mv(ex, ey), dn(),
                       {type:'pause', duration: held}, up()]);
    setStatus('tap sent (fallback)');
  } else {
    setStatus('tap ok (' + ex + ',' + ey + ')');
  }
});
